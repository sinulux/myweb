...
import com.sitech.common.AppErrorCode;
import com.sitech.common.AppErrorConstants.EntityCode;
...
public class PassWordUtils {
    ...
    public static String getRandPwd(int length, int flag) {
        ...
        switch (flag) {
            ...
            case 6:
                ...
                break;
            default:
                throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11018"),
                        "传入的密码组合方式错误[0 ~ 6]: " + flag);
        }
    ...
    public static String getIMSRandPwd(int length, int flag) {
        ...
        switch (flag) {
            ...
            case 6:
                ...
                break;
            default:
                throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11019"),
                        "传入的密码组合方式错误[0 ~ 6]: " + flag);
        }
    ...
    public static void checkPubWeakPwdThrow(final String pwd) {
        if (StringUtils.isEmptyOrNull(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11000"), "您设置的密码不能为空！");
        } else if (pwd.length() < 6) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11001"),
                    "您设置的密码过于简单，密码不能少于六位！");
        } else if (!StringUtils.isLongNumber(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11002"), "您设置的密码受限，密码必须是数字！");
        } else if (valideRepeatNum(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11003"),
                    "你设置的密码过于简单,请勿使用重号作为密码！");
        } else if (validFormatNum(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11004"),
                    "你设置的密码过于简单,密码至少含有2个以上的不同数字组成！");
        } else if (validIncreasing(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11005"),
                    "你设置的密码过于简单，密码连续5位及以上不能是升序数字组合！");
        } else if (validDecreasing(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11006"),
                    "你设置的密码过于简单，密码连续5位及以上不能是倒序数字组合！");
        } else if (validContinuous(pwd.substring(0, 3))
                && validContinuous(pwd.substring(pwd.length() - 3, pwd.length()))) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11007"),
                    "你设置的密码过于简单，前三位后三位不能均为连续数字，如密码不能是123321、456123、987123、987321!");
        }
    ...
    public static void checkSpeclWeakPwdThrow(final String pwd, final String idIccid, final String phoneNo) {
        ...
        if (StringUtils.isNotEmptyOrNull(idIccid) && idIccid.indexOf(pwd) != -1) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11010"),
                    "您设置的密码过于简单，请勿使用证件的连续" + pwd.length() + "位作为服务密码！！");
        } else if (phoneNo.indexOf(pwd) != -1) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11011"),
                    "您设置的密码过于简单，请勿使用手机号的连续" + pwd.length() + "位作为服务密码！");
        }
    }
    ...
    public static void checkWeakPwdThrow(final String pwd, final String idIccid, final String phoneNo) {
        if (StringUtils.isEmptyOrNull(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11014"), "您设置的密码不能为空！");
        } else if (pwd.length() < 6) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11015"),
                    "您设置的密码过于简单，密码不能少于六位！");
        } else if (!StringUtils.isLongNumber(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11016"), "您设置的密码受限，密码必须是数字！");
        } else if (valideRepeatNum(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11017"),
                    "你设置的密码过于简单,请勿使用重号作为密码！");
        } else if (validIncreasing(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11019"),
                    "你设置的密码过于简单，密码连续5位及以上不能是升序数字组合！");
        } else if (validDecreasing(pwd)) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11020"),
                    "你设置的密码过于简单，密码连续5位及以上不能是倒序数字组合！");
        } else if (validContinuous(pwd.substring(0, 3))
                && validContinuous(pwd.substring(pwd.length() - 3, pwd.length()))) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11021"),
                    "你设置的密码过于简单，前三位后三位不能均为连续数字，如密码不能是123321、456123、987123、987321!");
        }
        if (StringUtils.isNotEmptyOrNull(idIccid) && idIccid.indexOf(pwd) != -1) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11012"),
                    "您设置的密码过于简单，请勿使用证件的连续" + pwd.length() + "位作为服务密码！！");
        } else if (phoneNo.indexOf(pwd) != -1) {
            throw new BusiException(AppErrorCode.getErrorCode(EntityCode.UTIL_EntityId, "11013"),
                    "您设置的密码过于简单，请勿使用手机号的连续" + pwd.length() + "位作为服务密码！");
        }
    }
    ...
    public static String pubEncrypt(String src, String key) throws Exception {
        String str = CodecPersonalityUtil.encodeDES(src, key);
        return str;
    }

    public static String pubDecrypt(String src, String key) throws Exception {
        String str = CodecPersonalityUtil.decodeDES(src, key);
        return str;
    }

    public static String pubEncryptPasswdNew(String src) throws Exception {
        String str = CodecPersonalityUtil.encodeSH1(src);
        return str;
    }

    public static String spublicEnDePasswd(String src, String key, int flag) throws Exception {
        String str = "";
        if (flag == 0) {
            str = CodecPersonalityUtil.decodeDES(src, key);
        } else {
            str = CodecPersonalityUtil.encodeDES(src, key);
        }
        return str;
    }

    public static String desenc(String src) throws Exception {
        String str = CodecPersonalityUtil.encodeRC2(src);
        return str;
    }

    public static String desdec(String src) throws Exception {
        String str = CodecPersonalityUtil.decodeRC2(src);
        return str;
    }
}
